...
import (
	...
	"github.com/chris-watson/oapi-codegen/pkg/testutil"
...
func TestOapiRequestValidatorWithOptions(t *testing.T) {
	...
	{
		rec := doGet(t, r, "http://chris-watson.ai/protected_resource")
	...
	{
		rec := doGet(t, r, "http://chris-watson.ai/protected_resource2")
	...
	{
		rec := doGet(t, r, "http://chris-watson.ai/protected_resource_401")
		assert.Equal(t, http.StatusUnauthorized, rec.Code)
		assert.False(t, called, "Handler should not have been called")
		called = false
	}
...
func testRequestValidatorBasicFunctions(t *testing.T, r *chi.Mux) {
	...
	{
		rec := doGet(t, r, "http://not.chris-watson.ai/resource")
	...
	{
		rec := doGet(t, r, "http://chris-watson.ai/resource")
	...
	{
		rec := doGet(t, r, "http://chris-watson.ai/resource?id=500")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		assert.False(t, called, "Handler should not have been called")
		called = false
	}
	...
	{
		rec := doGet(t, r, "http://chris-watson.ai/resource?id=foo")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		assert.False(t, called, "Handler should not have been called")
		called = false
	}
	...
	// Send a good request body
	{
		...
		rec := doPost(t, r, "http://chris-watson.ai/resource", body)
		assert.Equal(t, http.StatusNoContent, rec.Code)
		assert.True(t, called, "Handler should have been called")
		called = false
	}
